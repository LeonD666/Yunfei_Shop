...
// ==============================================
// 2. 提交按钮时执行：验证 + 存储缓存 + 直接读取缓存 + 上传 + 反馈 +反馈成功后删除缓存
// ==============================================
async function submitInfo() {
  try {
    ...
    // 3. 表单验证
    if(!name){
        tip.innerText = '客户名称不能为空，请填写，可以填写昵称';
        tip.style.color = 'red';
        return;
    }
    ...
    // 5. 直接读取上一页的缓存
    const pay_order = sessionStorage.getItem('pay_order');
    const total_price = sessionStorage.getItem('total_price');
    const cart_list = sessionStorage.getItem('cart_out');


    // 6. 提交到 Edge Function 再转到数据库
    const formData = {
      eat_type,
      name,
      phone,
    ...
    if (res.ok) { 
      ...
      // 提交成功后，清空缓存内容
      sessionStorage.removeItem('cart_out');
